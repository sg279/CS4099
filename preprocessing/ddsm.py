...
def ddsm_test_metadata_conversion():
    ...
    images_list = [i.split("\\")[5][:-4] for i in absoluteFilePaths('F:\\DDSM data\\pngs_corrected\\test')]
    # data = data.loc[data["image file path"].duplicated(keep='last')!=True]
    ...
    for row in data.iterrows():
        # if row[1]["ROI mask file path"].split("/")[0][-1] == '2':
        #     continue
        i = i + 1
        ...
        test_metadata.loc[label] = [pathology, label, density, side, view, abnormality_id, abnormality_type, shape, margins,
                                          assessment, subtlety, calc_distribution, calc_type]
...
def process_radiologist_metadata():
    ...
    cat_columns = ['shape', 'margins', 'calc_distribution','calc_type']
    num_columns = ['density','assessment', 'subtlety']
    minMax = MinMaxScaler()
    encoded = None
    ...
    training.to_csv("./all_training_metadata.csv", index=False)
    val.to_csv("./all_val_metadata.csv", index=False)
    test.to_csv("./all_test_metadata.csv", index=False)
...
def ddsm_cropped_test_conversion():
    data = pd.read_csv("../mass_case_description_test_set.csv").append(pd.read_csv("../calc_case_description_test_set.csv"))
    data["pathology"].replace("MALIGNANT", "M", inplace=True)
    data["pathology"].replace(["BENIGN","BENIGN_WITHOUT_CALLBACK" ], "B", inplace=True)
    ...
    for row in data.iterrows():
        ...
        subfolder1 = os.listdir(os.path.join(source_path, image_folder))[0]
        subfolder2 = os.listdir(os.path.join(source_path, image_folder, subfolder1))[0]
        file = sorted(os.listdir(os.path.join(source_path, image_folder, subfolder1, subfolder2)),
                      key=lambda filename: os.path.getsize(
                          os.path.join(source_path, image_folder, subfolder1, subfolder2, filename)))[0]
        ds = dicom.dcmread(os.path.join(source_path, image_folder, subfolder1, subfolder2, file))
        pixel_array_numpy = ds.pixel_array
        image = image_folder+".png"
        cv2.imwrite(os.path.join(path,"test",row[1]["pathology"], image), pixel_array_numpy)

def ddsm_cropped_train_conversion():
    mass_data = pd.read_csv("../mass_case_description_train_set.csv")
    calc_data = pd.read_csv("../calc_case_description_train_set.csv")
    mass_cutoff = round(mass_data.shape[0] * 0.2)
    calc_cutoff = round(calc_data.shape[0] * 0.2)
